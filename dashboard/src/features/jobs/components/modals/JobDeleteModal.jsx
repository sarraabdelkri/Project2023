...
const JobDeleteModal = ({ element, job, onDelete, openNow, onClose }) => {
  ...
  return (
    <Modal element={element} openNow={openNow} onClose={() => onClose()}>
      ...
      >
        <Trash size={22} className="mr-2" /> Deleting job {job.title}
      </Dialog.Title>
      <div className="mt-2">
        <p className="text-sm text-darker-t">
          The job {job.title} will be permanently deleted.
        </p>
        <p className="text-sm text-darker-t">
