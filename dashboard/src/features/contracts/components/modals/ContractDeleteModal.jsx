...
const ContractDeleteModal = ({ element, contract, onDelete, openNow, onClose }) => {
    ...
    return (
        <Modal element={element} openNow={openNow} onClose={() => onClose()}>
            ...
            >
                <Trash size={22} className="mr-2" /> Deleting contract {contract._id.slice(-6).toUpperCase()}
            </Dialog.Title>
            <div className="mt-2">
                <p className="text-sm text-darker-t">
                    The contract {contract._id.slice(-6).toUpperCase()} will be permanently deleted.
                </p>
                <p className="text-sm text-darker-t">
